...
const EventsByCity = ({ role }) => {
  ...
  useEffect(() => {
    city &&
      axios.post(`/api/events/getEventsByCity/${city}`).then((res) => {
        const tempEvents = res.data.filter(
          (item) => item.event_status === "confirm"
        );
        setEvents(tempEvents);
        setSelectedEvent(tempEvents[0]?._id);
      });
  }, [city]);
