...
function NecceseryEquipment({ event, cours }) {
  let coursObj = cours?.essentialEq;
  ...
  const filteredCoursItem =
    ...
    filteredCoursServices.map((service, i) => {
      return essentialList.find((item) => item.label == service);
    });

  let obj = event?.essentialEq;
      ...
      {!event && !cours ? (
        ...
        <>
          <Grid item style={{ padding: "10px" }}>
            <Typography variant="h6">تجهیزات مورد نیاز</Typography>
          </Grid>
...
export default NecceseryEquipment;
