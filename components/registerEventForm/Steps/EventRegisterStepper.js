...
import { eventSubmitValue } from "../../redux/features/eventSlice";
...
export default function EventRegisterStepper({
  children,
  editEvent,
  disableBtn,
  completedData,
  ...
  const handleComplete = () => {
    !editEvent
      ? // dispatch(eventSubmitValue(submitEventData));
        ...
        axios.post("/api/events/registerEvent", completedData).then((res) => {
          if (res.status == 200) {
            toast.success(` ${completedData.event_title} با موفقیت ایجاد شد `);
            router.push("/events");
          } else {
      ...
      : completedSteps() === totalSteps() - 1 &&
        axios
          ...
          .then((res) => {
            if (res.status == 200) {
              toast.success(
                ` ${completedData.event_title} با موفقیت ویرایش شد `
              );
          ...
          <div>
            {allStepsCompleted() ? (
              <ResultPage
                width="500"
                height="400"
                title="برنامه شما با موفقیت ثبت شد!"
